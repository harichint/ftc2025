 ...
 */
@TeleOp(name = "Main Program Velo", group = "Main")
...
//        rightIntakeGate.setPower(GATE_SERVO_POWER);
    ...
    private void handleMechanisms() {
        ...
        if (gamepad2.dpad_down && !dpad_down_was_pressed) {
            mechanismState = (mechanismState == SystemState.MANUAL_SHOOTING) ? SystemState.STOPPED : SystemState.MANUAL_SHOOTING;
        }
        ...
        switch (mechanismState) {
            ...
            case SHOOTING_SEQUENCE:
                ...
                break;
            case SHOOTER_ONLY:
                leftConveyorBelt.setVelocity(MAX_VELOCITY * 0.8);
                rightConveyorBelt.setVelocity(MAX_VELOCITY * 0.8);
                break;
            case LEFT_REVERSED:
                leftConveyorBelt.setVelocity(REVERSE_VELOCITY);
...
// Logic for Right side
    ...
    public void runManualNearShooting() {
        double targetVelocity = 1400.0;
        double powerToUse = 0.75;
        runShootingActually(targetVelocity, powerToUse);
    }
    ...
    public void runShootingSequence() {
        ...
        double powerToUse = 0;
        double powerRatio;        // 1. Logic for Target Velocity based on distance
        if (distInches > 80 || Double.isNaN(distInches)) {
            ...
            // CLOSE RANGE LOGIC:
            // At 50 inches and below, we need a more aggressive power floor.
